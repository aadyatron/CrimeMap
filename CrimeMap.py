...
# for graphs with year input (bar charts)
def graphs_year_choice(df,x):
    print("")
    yn=input("Do you wish to see the graphical representation of the above data? (y/n): ")
    if yn=='y' or yn=='Y':
        while True:
            # while loop is used to repeatedly prompt the user to enter a year until a valid input is received.
            year=input("enter a year (2020 to 2022): ")
        ...
        print("Invalid input, please retry...")
        return graphs_year_choice(df,x)     # recalls the function if 'yn' is invalid

# for general graphs (line plots)
def graphs(df,x,y):
    print("")
    yn=input("Do you wish to see the graphical representation of the above data? (y/n): ")
    if yn=='y' or yn=='Y':
...
# pages: each page is structured as a separate function for easy integration & looping.
def page1():
    pg1={
...
def page3():
    ...
    if choice=='1':
        ...
        print("\n\t\t\t- Total Crimes -")
        print(data[['City','2020','2021','2022']],"\n")
        graphs_year_choice(data,'City')
...
# while loop is used to continuously display the main menu unless the user selects option 7 : exit
while True:
    main_menu()
